...
test('happy flow', async ({ page }) => {
  ...
  await page.getByLabel('Hospital contact number').fill('Hospital contact number')
  ...
  await page.getByLabel('Date of service').fill('Date of service')
  await page.getByLabel('Diagnosis').fill('Diagnosis')
  await page.getByLabel('Total billed').fill('Total billed')
  await page.getByLabel('Reason').fill('Reason')
  await page.getByRole('button', { name: 'Next' }).click()

  // Consent Page
  await expect(page.getByRole('heading', { name: 'Terms and Conditions' })).toBeVisible()
  await page.getByLabel('I certify this information is true.').fill('I certify this information is true.')
  await page.getByLabel('I authorize release of medical information for application.').fill('I authorize release of medical information for application.')
